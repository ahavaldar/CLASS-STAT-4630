...
table(y.test, pred.bag)
mean(pred.bag==y.test) ##0.889. Improvement over pruning.

##change threshold
##obtain predicted probabilities
pred.bag2<-predict(bag.class, newdata=test, type="prob")
##confusion matrix for threshold of 0.8
thresh8<-table(y.test, pred.bag2[,2]>0.8)
thresh8
...
##confusion matrix
boost.tab<-table(y.test.dummy, pred.boost>0.5)
boost.tab
